			...
			case "csong" ->
			{
				...
				switch (args[0])
				{
					case "play" ->
					{
						...
						if (fileName != null && !fileName.endsWith(".nbs") && !fileName.startsWith("--random"))
						{
							fileName += ".nbs";
						}
						try
					...
					case "change-song" ->
					{
						...
						try
						{
							...
							if (fade)
							{
								...
								Bukkit.getScheduler().runTaskLater(Cucumbery.getPlugin(), () -> changeSongCooldown = false, 40);
								...
								int maxTick = 40;
								int midTick = maxTick / 2;
								for (int i = 1; i <= midTick - 1; i++)
								{
									int finalI = i;
									songChangeFade.add(Bukkit.getScheduler()
											.runTaskLater(Cucumbery.getPlugin(), () -> radioSongPlayer.setVolume((byte) (volume * (midTick - finalI) / midTick)), finalI));
								}
								songChangeFade.add(
										Bukkit.getScheduler().runTaskLater(Cucumbery.getPlugin(), () -> TICK_PERCENTAGE = 1d * radioSongPlayer.getTick() / songLength, midTick));
								songChangeFade.add(Bukkit.getScheduler().runTaskLater(Cucumbery.getPlugin(), () -> radioSongPlayer.playSong(0), midTick));
								boolean finalReset = reset;
								songChangeFade.add(Bukkit.getScheduler().runTaskLater(Cucumbery.getPlugin(),
										() -> radioSongPlayer.setTick(finalReset ? 0 : (short) (TICK_PERCENTAGE * radioSongPlayer.getSong().getLength())), midTick));
								for (int i = midTick + 1; i <= maxTick; i += 1)
								{
									int finalI = i;
									songChangeFade.add(Bukkit.getScheduler()
											.runTaskLater(Cucumbery.getPlugin(), () -> radioSongPlayer.setVolume((byte) (volume * (finalI * 2 - maxTick) / maxTick)), i));
								}
							}
							else
							{
					...
					case "info" ->
					{
						...
						MessageUtil.sendMessage(sender, Prefix.INFO_SONG, "길이 : %s",
					...
					case "play" ->
					{
						if (args.length < 4)
						{
							...
							MessageUtil.commandInfo(sender, label,
									Variable.ORIGINAL_NAME.getOrDefault(player.getUniqueId(), player.getName()) + " play <명령어 출력 숨김 여부> <노래 파일 이름>");
							return true;
						}
						if (!MessageUtil.isBoolean(sender, args, 3, true))
						{
			...
			@NotNull Location location)
	{
		...
		switch (cmd.getName())
		{
			case "csong" ->
			{
				if (length == 1)
				{
					return CommandTabUtil.tabCompleterList(args, "<인수>", false, Completion.completion("play", ComponentUtil.translate("서버 전체에 노래를 재생함니다")),
