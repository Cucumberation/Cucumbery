	...
	public void onPlayerInteract(PlayerInteractEvent event)
	{
		...
		if (CustomEffectManager.hasEffect(player, CustomEffectTypeRune.RUNE_USING) && ItemStackUtil.itemExists(player.getInventory().getItemInMainHand()))
		{
			...
			if (customEffect instanceof StringCustomEffect stringCustomEffect)
			{
				...
				{
					if (entity instanceof Item i && CustomEffectManager.getEffectNullable(i,
							CustomEffectTypeRune.RUNE_OCCUPIED) instanceof PlayerCustomEffect playerCustomEffect && playerCustomEffect.getPlayer().equals(player))
					{
						item = i;
					}
				}
		...
		if (!Cucumbery.config.getBoolean("grant-default-permission-to-players") && (blockClick || action == Action.PHYSICAL)
				&& !Permission.EVENT_INTERACT_BLOCK.has(player))
		{
			event.setCancelled(true);
			if (!Permission.EVENT_ERROR_HIDE.has(player) && !Variable.playerInteractAlertCooldown.contains(uuid))
			{
		...
		if (itemExists)
		{
			if (NBTAPI.isRestricted(player, item, RestrictionType.NO_USE) || (leftClick && NBTAPI.isRestricted(player, item, RestrictionType.NO_USE_LEFT_CLICK)) || (
					leftAir && NBTAPI.isRestricted(player, item, RestrictionType.NO_USE_LEFT_CLICK_AIR)) || (leftBlock && NBTAPI.isRestricted(player, item,
					RestrictionType.NO_USE_LEFT_CLICK_BLOCK)) || (rightClick && NBTAPI.isRestricted(player, item, RestrictionType.NO_USE_RIGHT_CLICK)) || (rightAir
					&& NBTAPI.isRestricted(player, item, RestrictionType.NO_USE_RIGHT_CLICK_AIR)) || (rightBlock && NBTAPI.isRestricted(player, item,
					RestrictionType.NO_USE_RIGHT_CLICK_BLOCK)) || (airClick && NBTAPI.isRestricted(player, item, RestrictionType.NO_USE_AIR_CLICK)) || (blockClick
					&& NBTAPI.isRestricted(player, item, RestrictionType.NO_USE_BLOCK_CLICK)))
			{
				event.setCancelled(true);
				if (!Permission.EVENT_ERROR_HIDE.has(player) && !Variable.playerInteractAlertCooldown.contains(uuid))
				{
			...
			if (rightClick)
			{
				...
				if ((player.getGameMode() == GameMode.SURVIVAL || player.getGameMode() == GameMode.ADVENTURE) && NBTAPI.isRestricted(player, item,
						RestrictionType.NO_EQUIP))
				{
				...
				if (rightBlock)
				{
					...
					if (CustomEffectManager.hasEffect(player, CustomEffectTypeCustomMining.CUSTOM_MINING_SPEED_MODE) && !CustomEffectManager.hasEffect(player,
							CustomEffectTypeCustomMining.CUSTOM_MINING_SPEED_MODE_2_NO_RESTORE) && player.getGameMode() != GameMode.CREATIVE
							&& itemType == Material.ARMOR_STAND && blockFace != BlockFace.DOWN)
					{
						event.setCancelled(true);
						return;
					}
					...
					// 물병 들고 흙에 우클릭 (진흙 만들기)
					if (itemType == Material.POTION && item.getItemMeta() instanceof PotionMeta potionMeta && potionMeta.getBasePotionType() == PotionType.WATER
							&& Tag.DIRT.isTagged(clickedBlockType))
					{
					...
					{
						if (clickedBlockType == Material.CAULDRON || clickedBlockType == Material.WATER_CAULDRON || clickedBlockType == Material.LAVA_CAULDRON
								|| clickedBlockType == Material.POWDER_SNOW_CAULDRON || clickedBlockType == Material.DIRT)
						{
							// 커스텀 채광 모드에서는 불가능하게 막음
							if (player.getGameMode() != GameMode.CREATIVE && CustomEffectManager.hasEffect(player, CustomEffectTypeCustomMining.CUSTOM_MINING_SPEED_MODE)
									&& !CustomEffectManager.hasEffect(player, CustomEffectTypeCustomMining.CUSTOM_MINING_SPEED_MODE_2_NO_RESTORE))
					...
					if ((itemType == Material.FLINT_AND_STEEL || itemType == Material.FIRE_CHARGE) && clickedBlockType == Material.TNT)
					{
						...
						if (!player.isSneaking() && Cucumbery.config.getBoolean("use-static-tnt") && !Method.configContainsLocation(block.getLocation(),
								Cucumbery.config.getStringList("no-use-static-tnt-location")))
						{
				...
				try
				{
					switch (customMaterial)
					{
						case THE_MUSIC ->
						{
							...
							{
								MessageUtil.sendWarn(player, "능력을 사용하려면 %s 더 기다려야합니다!",
										ComponentUtil.translate("rg255,204;%s초", Constant.Sosu2.format(customEffect.getDuration() / 20d)));
								return;
							}
		...
		if (rightClick)
		{
			...
			if (rightBlock)
			{
				...
				switch (clickedBlockType)
				{
					...
					case JUKEBOX ->
					{
						boolean noTrade = NBTAPI.isRestricted(player, item, RestrictionType.NO_TRADE), noJukeBox = NBTAPI.isRestricted(player, item,
								RestrictionType.NO_JUKEBOX);
		...
		if (action == Action.PHYSICAL && block != null)
		{
			...
			if (block.getType() == Material.FARMLAND)
			{
				...
				if ((Cucumbery.config.getBoolean("block-player-trample-soil") && !Method.configContainsLocation(block.getLocation(),
						Cucumbery.config.getStringList("no-block-player-trample-soil-worlds"))) || UserData.TRAMPLE_SOIL_FORCE.getBoolean(uuid))
				{
					if (UserData.TRAMPLE_SOIL.getBoolean(uuid) || UserData.TRAMPLE_SOIL_FORCE.getBoolean(uuid))
					{
						event.setUseInteractedBlock(Event.Result.DENY);
						event.setCancelled(true);
						if (UserData.TRAMPLE_SOIL_ALERT.getBoolean(uuid) && !Permission.EVENT_ERROR_HIDE.has(player) && !UserData.TRAMPLE_SOIL_NO_ALERT_FORCE.getBoolean(
								uuid) && !Variable.soilTrampleAlertCooldown.contains(uuid))
						{
			...
			for (int i = 0; i < range * 8; i++)
			{
				...
				for (Entity entity : player.getWorld().getNearbyEntities(origin, 0.1D * width, 0.1D * width, 0.1D * width))
				{
					...
					if (entity instanceof LivingEntity livingEntity && (suicide || entity != player))
					{
						...
						{
							if (!(livingEntity instanceof Player && ((Player) livingEntity).getGameMode() == GameMode.SPECTATOR) && (ignoreInvincible || (
									!(livingEntity instanceof Player) || ((Player) livingEntity).getGameMode() != GameMode.CREATIVE)))
							{
								current++;
							}
								...
								else
								{
									switch (fireworkType)
									{
										...
										case 3 ->
										{
											...
											FireworkEffect effect = FireworkEffect.builder().with(fireworkEffectType).withColor(Color.YELLOW).withFade(Color.WHITE).flicker(true)
													.build();
											fwM.addEffect(effect);
										}
										case 4 ->
										...
										case 8 ->
										{
											...
											fwM.addEffect(effect);
											fireworkEffectType = FireworkEffect.Type.BALL_LARGE;
											effect = FireworkEffect.builder().with(fireworkEffectType)
													.withColor(Color.fromRGB(60, 0, 60), Color.fromRGB(80, 0, 80), Color.fromRGB(100, 0, 100)).build();
											fwM.addEffect(effect);
										}
										case 9 ->
										{
											FireworkEffect.Type fireworkEffectType = FireworkEffect.Type.BURST;
											FireworkEffect effect = FireworkEffect.builder().with(fireworkEffectType).withColor(Color.RED, Color.YELLOW, Color.ORANGE).flicker(true)
													.trail(true).build();
											fwM.addEffect(effect);
											fireworkEffectType = FireworkEffect.Type.CREEPER;
											effect = FireworkEffect.builder().with(fireworkEffectType).withColor(Color.BLACK).trail(true).build();
