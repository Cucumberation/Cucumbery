...
public class BlockBreak implements Listener
{
	...
	public void onBlockBreak(BlockBreakEvent event)
	{
		...
		if (Variable.scrollReinforcing.contains(uuid))
		{
			...
			Component a = ComponentUtil.create(Prefix.INFO, "만약 아이템 강화를 중지하시려면 이 문장을 클릭해주세요.").hoverEvent(ComponentUtil.create("클릭하면 강화를 중지합니다"))
					.clickEvent(ClickEvent.runCommand("/강화 quit"));
		...
		{
			if (blockType == Material.PUMPKIN_STEM || blockType == Material.MELON_STEM
		...
		boolean isTelekinesis =
				CustomEnchant.isEnabled() && (itemMeta != null && itemMeta.hasEnchant(CustomEnchant.TELEKINESIS)) || CustomEffectManager.hasEffect(player,
						CustomEffectType.TELEKINESIS), isSmeltingTouch =
				CustomEnchant.isEnabled() && (itemMeta != null && itemMeta.hasEnchant(CustomEnchant.SMELTING_TOUCH)) || CustomEffectManager.hasEffect(player,
						CustomEffectType.SMELTING_TOUCH), isSilkTouch = CustomEffectManager.hasEffect(player, CustomEffectType.SILK_TOUCH);
		...
		int harvestingLevel =
				CustomEnchant.isEnabled() && itemMeta != null && itemMeta.hasEnchant(CustomEnchant.HARVESTING) ? itemMeta.getEnchantLevel(CustomEnchant.HARVESTING) : 0;
		if (harvestingLevel > 0 && (!(block.getBlockData() instanceof Ageable ageable) || ageable.getAge() == ageable.getMaximumAge()))
		{
			farmingFortune += harvestingLevel * 0.15f;
		}

		// 커스텀 채광 모드에서는 즉시 부서지는 작물(밀, 당근 등. 호박, 코코아 콩 등은 제외)을 캘 때 행운 대신 전용 인챈트 사용
		if (CustomEffectManager.hasEffect(player, CustomEffectTypeCustomMining.CUSTOM_MINING_SPEED_MODE_2_NO_RESTORE) && Tag.CROPS.isTagged(block.getType())
		...
		if (drops.size() == 1)
		{
			...
			if (dataString != null)
			{
				...
				if (nbtItem.getCompound("displays") != null && nbtItem.getCompound("displays").hasTag("rotation") && (nbtItem.hasTag("perspectiveYaw") || nbtItem.hasTag(
						"perspectivePitch") || nbtItem.hasTag("perspectiveGrid")))
				{
		...
		if (vanilliaDrop)
		{
			if (event.isDropItems())
			{
				...
				if (isTelekinesis || isSmeltingTouch || isSilkTouch)
				{
					...
					{
						event.setCancelled(true);
						Bukkit.getServer().getScheduler().runTaskLater(Cucumbery.getPlugin(), () -> block.setBlockData(Bukkit.createBlockData(Material.AIR)), 0L);
						...
						if (!isUnskilledTouch)
						{
							for (Double dropExp : dropsExp)
							{
								...
								if (guaranteeXp > 0)
								{
									...
									player.getWorld().spawnEntity(block.getLocation(), EntityType.EXPERIENCE_ORB, SpawnReason.CUSTOM,
											e -> ((ExperienceOrb) e).setExperience(finalGuaranteeXp));
								}
							}
						}
						if (!isTelekinesis)
				...
				if (!blockPlaceDataApplied && !isCoarseTouch && (CustomEnchant.isEnabled() && itemMeta != null && itemMeta.hasEnchant(CustomEnchant.COLD_TOUCH)
						&& itemMeta.hasEnchant(Enchantment.SILK_TOUCH)))
				{
					switch (blockType)
					{
				...
				else if (!isCoarseTouch && (itemMeta != null && CustomEnchant.isEnabled() && itemMeta.hasEnchant(CustomEnchant.WARM_TOUCH))
