...
public class BlockPlace implements Listener
{
	...
	public void onBlockPlace(BlockPlaceEvent event)
	{
		...
		if (CustomEffectManager.hasEffect(player, CustomEffectType.CURSE_OF_CREATIVITY) || CustomEffectManager.hasEffect(player,
				CustomEffectType.CURSE_OF_CREATIVITY_PLACE))
		{
			event.setCancelled(true);
			return;
		}
		...
		if (item.getType() == Material.TNT)
		{
			...
			if (tntTag != null)
			{
				if (tntTag.hasTag(CucumberyTag.TNT_IGNITE) && tntTag.getBoolean(CucumberyTag.TNT_IGNITE))
				{
					...
					if (Cucumbery.config.getBoolean("use-static-tnt") && !Method.configContainsLocation(block.getLocation(),
							Cucumbery.config.getStringList("no-use-static-tnt-location")))
					{
						tnt.setVelocity(new Vector(Cucumbery.config.getDouble("static-tnt-velocity.x"), Cucumbery.config.getDouble("static-tnt-velocity.y"),
								Cucumbery.config.getDouble("static-tnt-velocity.z")));
					}
				...
				Cucumbery.config.getStringList("no-use-block-place-data-feature-location")))
		{
			...
			if (item.hasItemMeta() && (NBTAPI.arrayContainsValue(extraTag, Constant.ExtraTag.PRESERVE_BLOCK_NBT) || !Cucumbery.config.getBoolean(
					"block-place-data-feature-with-specific-tag")))
			{
				item.setAmount(1);
				boolean perspectiveYaw =
						nbtItem.hasTag("perspectiveYaw") && nbtItem.getType("perspectiveYaw") == NBTType.NBTTagByte && nbtItem.getBoolean("perspectiveYaw");
				boolean perspectivePitch =
						nbtItem.hasTag("perspectivePitch") && nbtItem.getType("perspectivePitch") == NBTType.NBTTagByte && nbtItem.getBoolean("perspectivePitch");
				boolean reverseYaw = nbtItem.hasTag("reverseYaw") && nbtItem.getType("reverseYaw") == NBTType.NBTTagByte && nbtItem.getBoolean("reverseYaw");
				boolean reversePitch = nbtItem.hasTag("reversePitch") && nbtItem.getType("reversePitch") == NBTType.NBTTagByte && nbtItem.getBoolean("reversePitch");
				float additionalYaw =
						nbtItem.hasTag("additionalYaw") && nbtItem.getType("additionalYaw") == NBTType.NBTTagFloat ? nbtItem.getFloat("additionalYaw") : 0f;
				float additionalPitch =
						nbtItem.hasTag("additionalPitch") && nbtItem.getType("additionalPitch") == NBTType.NBTTagFloat ? nbtItem.getFloat("additionalPitch") : 0f;
				if (perspectiveYaw)
				{
				...
				if (perspectivePitch)
				{
					...
					if (additionalPitch != 0f)
					{
						pitch += additionalPitch;
					}
					if (pitch > 180f)
						pitch -= 360f;
					if (pitch < -180f)
				...
				if (nbtItem.hasTag("perspectiveGrid") && nbtItem.getType("perspectiveGrid") == NBTType.NBTTagByte && nbtItem.getBoolean("perspectiveGrid"))
				{
					...
					if (rotation.isEmpty())
					{
						rotation.add(perspectiveYaw ? yaw : 0f);
						if (perspectivePitch)
							rotation.add(pitch);
					}
					else
					{
						rotation.set(0, perspectiveYaw ? yaw : 0f);
						if (perspectivePitch)
							rotation.set(1, pitch);
					}
				}
