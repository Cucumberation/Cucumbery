...
public class EntityCustomEffectRemove implements Listener
{
	...
	public void onEntityCustomEffectRemove(EntityCustomEffectRemoveEvent event)
	{
		...
		if (entity instanceof Player player && customEffect.getDisplayType() == DisplayType.PLAYER_LIST && CustomEffectManager.getEffects(entity,
				DisplayType.PLAYER_LIST).isEmpty())
		{
		...
		if (entity instanceof Player player && (customEffectType == CustomEffectType.FLY || customEffectType == CustomEffectType.FLY_REMOVE_ON_QUIT)
		...
		if (customEffectType == CustomEffectType.FLY_NOT_ENABLED && entity instanceof Player player && player.getGameMode() != GameMode.CREATIVE
				&& player.getGameMode() != GameMode.SPECTATOR && (CustomEffectManager.hasEffect(entity, CustomEffectType.FLY) || CustomEffectManager.hasEffect(entity,
				CustomEffectType.FLY_REMOVE_ON_QUIT)))
		{
			CustomEffect effect = CustomEffectManager.hasEffect(entity, CustomEffectType.FLY)
					? CustomEffectManager.getEffect(entity, CustomEffectType.FLY)
					: CustomEffectManager.getEffect(entity, CustomEffectType.FLY_REMOVE_ON_QUIT);
						...
						if (attributeListMap.containsKey(Attribute.CAMERA_DISTANCE))
						{
							...
							if (!amountList.isEmpty())
							{
								if (entity instanceof Player player && customEffectType == CustomEffectType.SECRET_GUARD_EFFECT)
								{
									...
									{
										double finalAmount = amount * (loopCount - i) / loopCount;
									...
									for (int i = 0; i < loopCount; i++)
									{
										...
										TimerTask task = new TimerTask()
										{
											...
											public void run()
											{
												...
												var attributes = Lists.newArrayList(
														WrappedAttribute.newBuilder().attributeKey("scale").baseValue(attributeInstance.getBaseValue()).addModifier(
																WrappedAttributeModifier.newBuilder().key(namespacedKey.getNamespace(), namespacedKey.getKey())
																		.operation(WrappedAttributeModifier.Operation.ADD_NUMBER).amount(finalAmount).build()).build());
												packet.getIntegers().
		...
		if (customEffectType == CustomEffectType.STRANGE_CREATIVE_MODE && entity instanceof Player player
				&& customEffect instanceof StringCustomEffect stringCustomEffect)
		{
