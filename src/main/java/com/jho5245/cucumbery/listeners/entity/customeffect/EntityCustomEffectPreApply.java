	...
	public void onEntityCustomEffectPreApply(EntityCustomEffectPreApplyEvent event)
	{
		...
		if (customEffectType == CustomEffectType.RESURRECTION && CustomEffectManager.hasEffect(entity, CustomEffectType.RESURRECTION_COOLDOWN))
		{
			...
			MessageUtil.sendWarn(entity, "%s 효과가 적용중이여서 %s 효과를 적용받지 못했습니다",
					CustomEffectManager.getDisplay(entity, Collections.singletonList(CustomEffectManager.getEffect(entity, CustomEffectType.RESURRECTION_COOLDOWN)),
							true), customEffect);
			return;
		}

		if (customEffectType == CustomEffectType.FLY_NOT_ENABLED && !(CustomEffectManager.hasEffect(entity, CustomEffectType.FLY) || CustomEffectManager.hasEffect(
				entity, CustomEffectType.FLY_REMOVE_ON_QUIT)))
		{
			event.setCancelled(true);
			return;
		}
		...
		{
			if (!((entity instanceof Player player && player.getGameMode() != GameMode.SPECTATOR) || (entity instanceof Tameable tameable
