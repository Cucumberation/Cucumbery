		...
		for (CustomEffect customEffect : customEffects)
		{
			if (customEffect instanceof RealDurationCustomEffect realDurationCustomEffect)
			{
				...
				if (customEffect.getDuration() != -1)
				{
					...
					// 바닐라 효과의 커스텀 버전 중 접속을 종료해도 지속 시간이 경과하는 효과의 시간 업데이트
					// TODO: 농도 레벨이 여러 개일 경우 다른 레벨의 효과가 사라지는 현상 해결해야함 (Paper 측에서 구현 필요)
					if (entity instanceof LivingEntity livingEntity)
					{
						NamespacedKey namespacedKey = customEffect.getType().getNamespacedKey();
...
/*								List<PotionEffect> hiddenEffects = new ArrayList<>();
	...
	public static void display(@NotNull Player player)
	{
		...
		if (!showPotionEffect && customEffects.isEmpty())
		{
			if (Cucumbery.config.getBoolean("show-boss-bar-when-no-effects"))
			{
				...
				player.showBossBar(bossBar.color(Color.WHITE).progress(0f).name(ComponentUtil.translate("&7적용 중인 효과 없음")));
			}
			else
			{
				if (Variable.customEffectBossBarMap.containsKey(uuid))
		...
		if (!customEffects.isEmpty())
		{
			...
			component = component.append(ComponentUtil.translate("rg255,204;적용 중인 효과 목록 : %s개", "&2" + customEffects.size()));
			component = component.append(Component.text("\n"));
			for (CustomEffect customEffect : customEffects)
			{
				int duration = customEffect.getDuration();
				...
				component = component.append(Component.text("\n"));
				component = component.append(ComponentUtil.translate(amplifier == 0 ? "%1$s%2$s" : "%1$s %3$s%2$s", customEffect,
	...
	public static void displayGUI(@NotNull Player player)
	{
		...
		if (GUIManager.isGUITitle(title) && title instanceof TranslatableComponent translatableComponent && translatableComponent.args()
				.get(1) instanceof TextComponent textComponent && textComponent.content().equals(Constant.POTION_EFFECTS))
		{
	...
	{
		if (CustomEffectManager.hasEffect(player, CustomEffectType.FLY_NOT_ENABLED) && !(player.getGameMode() == GameMode.CREATIVE
	...
	public static void ascension(@NotNull Entity entity)
	{
		if (CustomEffectManager.hasEffect(entity, CustomEffectType.ASCENSION))
		{
			...
			{
				Bukkit.getScheduler().runTaskLater(Cucumbery.getPlugin(), () -> CustomEffectManager.addEffect(entity, CustomEffectType.ASCENSION_COOLDOWN), 0L);
				return;
			}
	...
	public static void superiorLevitation(@NotNull Entity entity)
	{
		...
		if (customEffect != null)
		{
			...
			Vector vector = entity.getVelocity();
			entity.teleportAsync(new Location(location.getWorld(), location.getX(), location.getY() + (amplifier + 1) * 0.9 / 20, location.getZ(), location.getYaw(),
					location.getPitch()));
	...
	public static void rune(@NotNull Player player)
	{
		...
		if (customEffect instanceof StringCustomEffect stringCustomEffect)
		{
			...
			MessageUtil.sendTitle(player, s, ItemStackUtil.itemExists(player.getInventory().getItemInMainHand())
					? "룬을 해방하려면 마우스를 순서대로 클릭하세요. R = 우클릭, L = 좌클릭"
					: "&c룬을 해방하려면 손에 아이템을 들고 있어야 합니다! R = 우클릭, L = 좌클릭", 0, 3, 0);
		}
	}
	...
	{
		if (CustomEffectManager.hasEffect(player, CustomEffectType.MASTER_OF_FISHING) || CustomEffectManager.hasEffect(player,
				CustomEffectType.MASTER_OF_FISHING_D))
		{
	...
	public static void starCatch(@NotNull Player player)
	{
		if (CustomEffectManager.hasEffect(player, CustomEffectTypeReinforce.STAR_CATCH_PREPARE))
		{
			...
			MessageUtil.sendTitle(player, ComponentUtil.translate("rg255,204;스타캐치"),
					ComponentUtil.translate(penalty != null && penalty >= 20 ? "연속해서 강화를 시도하면 스타캐치 난이도가 증가합니다" : "별이 초록색일 때 스타캐치 클릭하면 강화 성공률 증가!"), 0, 100, 0);
		}
		if (CustomEffectManager.hasEffect(player, CustomEffectTypeReinforce.STAR_CATCH_PROCESS))
		{
			...
			meh = new String(m);
			MessageUtil.sendTitle(player, ComponentUtil.translate(meh.replace("★", "%s").replace("-", "─"), (ok ? "&a" : "&c") + "★"),
					Constant.THE_COLOR_HEX + Constant.Jeongsu.format(duration / 20), 0, 5, 0);
		}
	}
		...
		if (CustomEffectManager.hasEffect(player, CustomEffectType.SPREAD) && player.getGameMode() != GameMode.SPECTATOR)
		{
			...
			if (!spreadTimerSet.contains(uuid))
			{
				...
				if (Math.random() < 0.0008)
				{
					...
					{
						List<CustomEffectType> effectTypes = new ArrayList<>(
								Arrays.asList(CustomEffectType.VAR_PNEUMONIA, CustomEffectType.VAR_PODAGRA, CustomEffectType.VAR_DETOXICATE, CustomEffectType.VAR_STOMACHACHE));
						if (detoxicate)
						{
						...
						CustomEffectManager.addEffect(player,
								new CustomEffect(effectTypes.get((int) (Math.random() * effectTypes.size())), spreadDuration, spreadAmplifier));
					}
			...
			{
				boolean skip =
						player.getGameMode() == GameMode.CREATIVE || player.getGameMode() == GameMode.SPECTATOR || player.hasPotionEffect(PotionEffectType.WATER_BREATHING);
	...
	public static void trueInvisibility(@NotNull Entity entity)
	{
		...
		if (scoreboardTags.contains("true_invisibility") || CustomEffectManager.hasEffect(entity, CustomEffectType.TRUE_INVISIBILITY))
		{
			for (Player player : Bukkit.getOnlinePlayers())
			{
				...
				{
					Bukkit.getScheduler().runTaskLater(Cucumbery.getPlugin(), () -> player.hideEntity(Cucumbery.getPlugin(), entity), 0L);
				}
			}
		}
		else
		{
			for (Player player : Bukkit.getOnlinePlayers())
			{
				if (entity != player && !player.canSee(entity))
				{
					Bukkit.getScheduler().runTaskLater(Cucumbery.getPlugin(), () -> player.showEntity(Cucumbery.getPlugin(), entity), 0L);
				}
			}
		}
	}
	...
	public static void darknessTerror(@NotNull Player player)
	{
		...
		if (CustomEffectManager.hasEffect(player, CustomEffectType.DARKNESS_TERROR) && location.getWorld().getEnvironment() == Environment.NORMAL
				...
				&& !location.getBlock().isSolid() && location.getBlock().getLightLevel() == 0 && !(Constant.OPTIFINE_DYNAMIC_LIGHT_ITEMS.contains(mainHand)
				|| Constant.OPTIFINE_DYNAMIC_LIGHT_ITEMS.contains(offHand)))
		{
	...
	public static void serverRadio(@NotNull Player player)
	{
		...
		else
		{
			...
			if (customEffect == null || customEffect.getAmplifier() != amplifier)
			{
				CustomEffectManager.removeEffect(player, CustomEffectType.SERVER_RADIO_LISTENING);
				CustomEffectManager.addEffect(player,
						new CustomEffect(CustomEffectType.SERVER_RADIO_LISTENING, CustomEffectType.SERVER_RADIO_LISTENING.getDefaultDuration(), amplifier));
			}
		}
	}
	...
	public static void axolotlsGrace(@NotNull Entity entity)
	{
		...
		for (Entity loop : entities)
		{
			...
			{
				Bukkit.getScheduler()
						.runTaskLater(Cucumbery.getPlugin(), () -> CustomEffectManager.addEffect(entity, new CustomEffect(CustomEffectType.PARROTS_CHEER)), 0L);
				break;
			}
		}
	}
	...
	public static void trollInventoryProperty(@NotNull Player player)
	{
		...
		{
			int min = CustomEffectManager.hasEffect(player, CustomEffectType.TROLL_INVENTORY_PROPERTY_MIN) ? CustomEffectManager.getEffect(player,
					CustomEffectType.TROLL_INVENTORY_PROPERTY_MIN).getAmplifier() : 0;
