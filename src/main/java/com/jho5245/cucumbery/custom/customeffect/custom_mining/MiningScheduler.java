...
public class MiningScheduler
{
	...
	public static void breakingBlock(@NotNull Player player)
	{
		breakingBlock(player, false, null);
	}
	...
	{
		if (isChain && chainLocation == null)
			throw new IllegalArgumentException("chainLocation is null when isChain is true");
		Location location;
		if (!isChain && CustomEffectManager.getEffectNullable(player,
				CustomEffectTypeCustomMining.CUSTOM_MINING_SPEED_MODE_PROGRESS) instanceof LocationCustomEffect effect)
		{
			location = effect.getLocation();
		}
		else
		{
			location = isChain ? chainLocation : null;
		}
		if (location == null)
		{
			return;
		...
		// 블록 채굴 대기시간 동안 채굴 불가
		if (!isChain && Variable.customMiningBlockBreakCooldown.contains(uuid))
		{
			MiningManager.quitCustomMining(player);
			return;
		}
		...
		if (isChain || progress >= 1 || instaBreak) // insta break
		{
			// 이벤트 호출
			CustomBlockBreakEvent customBlockBreakEvent = new CustomBlockBreakEvent(block, player, miningResult.applyPhysics(), isChain);
			...
			if (!playerCustomMiningItemDamageEvent.isCancelled() && !instaBreak && ItemStackUtil.itemExists(toolItemStack) && !toolItemStack.getItemMeta()
					.isUnbreakable())
			{
