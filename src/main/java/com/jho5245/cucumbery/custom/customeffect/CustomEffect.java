...
{
	protected final CustomEffectType effectType;

	protected final int initDuration;

	protected final int initAmplifier;
	...
	public CustomEffect(@NotNull CustomEffectType effectType, @Nullable OverridePropertyBuilder builder)
	{
		this(effectType, effectType.getDefaultDuration(), builder);
	}
	...
	public CustomEffect(@NotNull CustomEffectType effectType, int duration, @Nullable OverridePropertyBuilder builder)
	{
		this(effectType, duration, 0, builder);
	}
	...
	public CustomEffect(@NotNull CustomEffectType effectType, int duration, int amplifier, @Nullable OverridePropertyBuilder builder)
	{
		this(effectType, duration, amplifier, effectType.getDefaultDisplayType(), builder);
	}
	...
	public CustomEffect(@NotNull CustomEffectType effectType, int duration, int amplifier, @NotNull DisplayType displayType, @Nullable OverridePropertyBuilder builder)
	{
		...
		this.buffFreezable = builder != null && builder.buffFreezable != null ? builder.buffFreezable : effectType.isBuffFreezable();
		this.keepOnDeath = builder != null && builder.keepOnDeath != null ? builder.keepOnDeath : effectType.isKeepOnDeath();
		this.keepOnMilk = builder != null && builder.keepOnMilk != null ? builder.keepOnMilk : effectType.isKeepOnMilk();
		this.removeable = builder != null && builder.removeable != null ? builder.removeable : effectType.isRemoveable();
		this.keepOnQuit = builder != null && builder.keepOnQuit != null ? builder.keepOnQuit : effectType.isKeepOnQuit();
		this.realDuration = builder != null && builder.realDuration != null ? builder.realDuration : effectType.isRealDuration();
	}
	...
	{
		Component description = switch ((this.effectType.getNamespacedKey().getNamespace().equals("minecraft") ? "MINECRAFT_" : "") + effectType.getIdString()
				.split("__")[0].toUpperCase())
		{
			case "CURSE_OF_MUSHROOM" -> ComponentUtil.translate("%s 확률로 5초마다 인벤토리에 버섯이 들어옵니다", Constant.THE_COLOR_HEX + ((amplifier + 1) / 10d) + "%");
			case "FEATHER_FALLING" ->
					ComponentUtil.translate("낙하 피해를 받기 위한 최소 높이가 %sm 증가하고,", Constant.THE_COLOR_HEX + ((amplifier + 1) * 5)).append(Component.text("\n"))
			...
			case "BANE_OF_ARTHROPODS" ->
					ComponentUtil.translate("절지동물류 개체 공격 시 근거리 대미지가 %s 증가하고,", Constant.THE_COLOR_HEX + Constant.Sosu1.format((amplifier + 1) * 2.5))
							.append(Component.text("\n")).append(ComponentUtil.translate("%s 4단계 효과를 %s초간 부여합니다", ComponentUtil.translate("&ceffect.minecraft.slowness"),
									"&a1~" + Constant.Sosu1.format((amplifier + 3) * 0.5)));
			case "INSIDER" -> ComponentUtil.translate("채팅이 %s배로 입력되고, 죽을 때 모든 플레이어에게", amplifier + 2).append(Component.text("\n"))
					.append(ComponentUtil.translate("타이틀로 자신의 데스 메시지를 띄워줍니다"));
			case "OUTSIDER" -> ComponentUtil.translate("%s 확률로 채팅 메시지가 보내지지 않고", Constant.THE_COLOR_HEX + ((amplifier + 1) * 10) + "%").append(Component.text("\n"))
					.append(ComponentUtil.translate("입장 메시지, 퇴장 메시지가 뜨지 않습니다"));
			case "KINETIC_RESISTANCE" ->
					ComponentUtil.translate("겉날개 활강 중 블록에 부딪혀서 받는 피해량이 %s 감소됩니다", Constant.THE_COLOR_HEX + ((amplifier + 1) * 10) + "%").append(Component.text("\n"))
							.append(ComponentUtil.translate("낙하 피해량은 감소되지 않습니다"));
			case "ELYTRA_BOOSTER" -> ComponentUtil.translate("겉날개 활강 중 폭죽으로 가속할 때").append(Component.text("\n"))
					.append(ComponentUtil.translate("%s 확률로 폭죽을 소비하지 않습니다", Constant.THE_COLOR_HEX + ((amplifier + 1) * 10) + "%"));
			case "LEVITATION_RESISTANCE" ->
					ComponentUtil.translate("셜커에게 공격받아도 %s 확률로", Constant.THE_COLOR_HEX + ((amplifier + 1) * 10) + "%").append(Component.text("\n"))
			...
			case "NEWBIE_SHIELD" -> ComponentUtil.translate("플레이 시간이 1시간 미만인 당신!").append(Component.text("\n"))
			...
			case "WA_SANS" -> ComponentUtil.translate("스켈레톤 유형의 개체에게 받는").append(Component.text("\n")).append(
					ComponentUtil.translate("피해량이 %s 감소하고 대미지가 %s 증가합니다", Constant.THE_COLOR_HEX + ((amplifier + 1) * 3) + "%",
							Constant.THE_COLOR_HEX + ((amplifier + 1) * 10) + "%"));
			case "HEALTH_INCREASE" -> ComponentUtil.translate("최대 HP가 %s 증가합니다", Constant.THE_COLOR_HEX + ((amplifier + 1) * 10) + "%");
			case "SPREAD" -> ComponentUtil.translate("주변 %s블록 내의 다른 플레이어에게 자신이 가장 최근에 획득한 전이 효과를 옮깁니다", amplifier + 1).append(Component.text("\n"))
			...
			case "VAR_STOMACHACHE" ->
					ComponentUtil.translate("경험치를 획득하는 양만큼 오히려 경험치를 잃습니다").append(Component.text("\n")).append(ComponentUtil.translate("가지고 있는 경험치보다 잃는 경험치가 더 많을 경우"))
							.append(Component.text("\n")).append(ComponentUtil.translate("경험치 1당 대미지 0.1로 변환되어 들어옵니다")).append(Component.text("\n")).append(
									ComponentUtil.translate("추가로, 한번에 경험치 100 이상을 획득 시 %s 확률로 즉사합니다", Constant.THE_COLOR_HEX + Constant.Sosu1.format((amplifier + 1) * 0.1) + "%"));
			case "VAR_PNEUMONIA" ->
					...
					ComponentUtil.translate("%s, %s, %s, %s 상태 효과를 가지고 있을 경우", PotionEffectType.POISON, PotionEffectType.NAUSEA, PotionEffectType.BLINDNESS,
							...
							.append(Component.text("\n")).append(
									ComponentUtil.translate("%s 확률로 농도 레벨이 2단계가 낮아지거나 %s 확률로 3단계가 낮아질", Constant.THE_COLOR_HEX + (amplifier + 1) + "%",
											Constant.THE_COLOR_HEX + Constant.Sosu1.format((amplifier + 1) * 0.1) + "%")).append(Component.text("\n")).append(
									ComponentUtil.translate("수 있으며, %s 확률로 표시된 모든 디버프 4개가 제거될 수 있습니다",
											Constant.THE_COLOR_HEX + Constant.Sosu4.format((amplifier + 1) * (amplifier + 1) * 0.001) + "%"));
			case "VAR_PODAGRA" ->
			...
			case "ENDER_SLAYER" -> ComponentUtil.translate("%s, %s 또는 %s 공격 시 대미지가 %s 증가합니다", EntityType.ENDERMAN, EntityType.ENDERMITE, EntityType.ENDER_DRAGON,
					Constant.THE_COLOR_HEX + ((amplifier + 1) * 10) + "%");
		...
		if (this instanceof OfflinePlayerCustomEffect offlinePlayerCustomEffect)
		{
			...
			if (effectType == CustomEffectType.NEWBIE_SHIELD)
			{
				...
				description = description.append(
						ComponentUtil.translate("&7플레이 시간 : %s", Method.timeFormatMilli(player.getStatistic(Statistic.PLAY_ONE_MINUTE) * 50L, false)));
			}
		}
		...
		if (!excludeProperty)
		{
			...
			if (!effectType.getDescription().equals(Component.empty()) && (keepOnDeath || !keepOnQuit || keepOnMilk || !buffFreezable || isRealDuration))
			{
			...
			description = ComponentUtil.create(description, propertyDescription);
		}
		return description;
	}
	...
	{
		return effectType.isNegative();
	}

	public boolean doesCallEvent()
	{
		return effectType.doesCallEvent();
	}
	...
	{
		return new CustomEffect(getType(), getInitDuration(), getInitAmplifier(), getDisplayType(), builder);
	}
	...
	public static class OverridePropertyBuilder
	{
		protected Boolean buffFreezable, keepOnDeath, keepOnMilk, keepOnQuit, removeable, realDuration;

		public OverridePropertyBuilder buffFreezeable()
		{
			this.buffFreezable = true;
			return this;
		}

		public OverridePropertyBuilder nonBuffFreezeable()
		{
			this.buffFreezable = false;
			return this;
		}

		public OverridePropertyBuilder keepOnDeath()
		{
			this.keepOnDeath = true;
			return this;
		}

		public OverridePropertyBuilder removeOnDeath()
		{
			this.keepOnDeath = false;
			return this;
		}

		public OverridePropertyBuilder keepOnMilk()
		{
			this.keepOnMilk = true;
			return this;
		}

		public OverridePropertyBuilder removeOnMilk()
		{
			this.keepOnMilk = false;
			return this;
		}

		public OverridePropertyBuilder keepOnQuit()
		{
			this.keepOnQuit = true;
			return this;
		}

		public OverridePropertyBuilder removeOnQuit()
		{
			this.keepOnQuit = false;
			return this;
		}

		public OverridePropertyBuilder removeable()
		{
			this.removeable = true;
			return this;
		}

		public OverridePropertyBuilder nonRemoveable()
		{
			this.removeable = false;
			return this;
		}

		public OverridePropertyBuilder realDuration()
		{
			this.realDuration = true;
			return this;
		}

		public OverridePropertyBuilder nonRealDuration()
		{
