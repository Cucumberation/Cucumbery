		...
		switch (damageCause)
		{
			case ENTITY_ATTACK, ENTITY_SWEEP_ATTACK, ENTITY_EXPLOSION, CUSTOM, PROJECTILE ->
			{
				if (damageCause != DamageCause.PROJECTILE
						|| event instanceof EntityDamageByEntityEvent ev && ev.getDamager() instanceof Projectile projectile && projectile.getShooter() instanceof Entity)
				{
			...
			case ENTITY_ATTACK, ENTITY_SWEEP_ATTACK, CUSTOM ->
			{
				if (event instanceof EntityDamageByEntityEvent damageByEntityEvent)
				{
					...
					else
					{
						...
						if (CustomEffectManager.hasEffect(damagerEntity, CustomEffectType.SMITE) && (victim instanceof Zombie || victim instanceof ZombieHorse
								|| victim instanceof AbstractSkeleton || victim instanceof Wither))
						{
						...
						if (CustomEffectManager.hasEffect(damagerEntity, CustomEffectType.BANE_OF_ARTHROPODS) && (victim instanceof Spider || victim instanceof Silverfish
								|| victim instanceof Endermite))
						{
						...
						if (CustomEffectManager.hasEffect(damagerEntity, CustomEffectType.HEROS_ECHO) || CustomEffectManager.hasEffect(damagerEntity,
								CustomEffectType.HEROS_ECHO_OTHERS))
						{
						...
						if (CustomEffectManager.hasEffect(victim, CustomEffectType.COMBAT_MODE_RANGED) || CustomEffectManager.hasEffect(damagerEntity,
								CustomEffectType.COMBAT_MODE_MELEE))
						{
							damageMultiplier += 1d;
						}
						if (CustomEffectManager.hasEffect(damagerEntity, CustomEffectType.ENDER_SLAYER) && (victim instanceof Enderman || victim instanceof EnderDragon
								|| victim instanceof Endermite))
						{
							int amplifier = CustomEffectManager.getEffect(damagerEntity, CustomEffectType.ENDER_SLAYER).getAmplifier();
							damageMultiplier += (amplifier + 1) * 0.1;
						}
						if (CustomEffectManager.hasEffect(damagerEntity, CustomEffectType.BOSS_SLAYER) && (victim instanceof Boss || victim.getScoreboardTags()
								.contains("boss") || MythicMobManager.hasTag(victim, "boss")))
						{
					...
					if (damagerEntity instanceof LivingEntity livingEntity)
					{
						...
						if (equipment != null)
						{
							...
							if (CustomEnchant.isEnabled() && ItemStackUtil.itemExists(mainHand) && mainHand.hasItemMeta() && mainHand.getItemMeta()
									.hasEnchant(CustomEnchant.CLEAVING))
							{
				...
				if (event instanceof EntityDamageByEntityEvent damageByEntityEvent)
				{
					...
					if (damagerEntity instanceof Projectile projectile)
					{
						...
						if (projectileSource instanceof Entity projectileDamager)
						{
							...
							if (CustomEffectManager.hasEffect(projectileDamager, CustomEffectType.HEROS_ECHO) || CustomEffectManager.hasEffect(projectileDamager,
									CustomEffectType.HEROS_ECHO_OTHERS))
							{
								finalDamageMultiplier *= 1.05;
							}
							...
							if (CustomEffectManager.hasEffect(victim, CustomEffectType.COMBAT_MODE_MELEE) || CustomEffectManager.hasEffect(projectileDamager,
									CustomEffectType.COMBAT_MODE_RANGED))
							{
								damageMultiplier += 1d;
							}
							if (CustomEffectManager.hasEffect(projectileDamager, CustomEffectType.ENDER_SLAYER) && (victim instanceof Enderman
									|| victim instanceof EnderDragon || victim instanceof Endermite))
							{
								int amplifier = CustomEffectManager.getEffect(projectileDamager, CustomEffectType.ENDER_SLAYER).getAmplifier();
								damageMultiplier += (amplifier + 1) * 0.1;
							}
							if (CustomEffectManager.hasEffect(projectileDamager, CustomEffectType.BOSS_SLAYER) && (victim instanceof Boss || victim.getScoreboardTags()
									.contains("boss") || MythicMobManager.hasTag(victim, "boss")))
							{
