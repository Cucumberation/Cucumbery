...
public class CommandTabUtil
{
	...
	public static final List<Completion> ARGS_LONG = Collections.singletonList(
			Completion.completion(Prefix.ARGS_LONG.toString(), ComponentUtil.translate(Prefix.ARGS_LONG.toString())));
	...
	public static List<String> customItemTabCompleter(Player player, String[] args)
	{
		...
		switch (customItemType)
		{
			case CucumberyTag.CUSTOM_ITEM_RAILGUN_ID:
				if (length == 3)
				{
					return Method.tabCompleterList(args, "<태그>", "range", "sortparticle", "ignoreinvincible", "density", "damage", "blockpenetrate",
							"fireworkrocketrequired", "cooldown", "cooldowntag", "piercing", "laserwidth", "fireworktype", "reverse", "suicide", "particle-type");
				}
				else if (length == 4)
				{
					switch (args[2])
	...
	@NotNull
	public static List<Completion> fromLegacy(@NotNull TabCompleter completer, @NotNull CommandSender sender, @NotNull Command cmd, @NotNull String label,
			@NotNull String[] args)
	{
	...
	{
		return completions.size() == 1 && completions.get(0).tooltip() instanceof TranslatableComponent translatableComponent && (
				translatableComponent.args().size() == 2 && translatableComponent.args().get(1) instanceof TextComponent t && t.content().equals("error-text"));
	}
	 ...
	 * @param lastArg
	 * 		명령어의 마지막 인수
	 * @param excludeNonPlayerEntity
	 * 		바라보고 있는 개체의 UUID 중 플레이어가 아닌 개체의 UUID 제외 여부
	...
	public static List<Completion> tabCompleterOfflinePlayer(@NotNull CommandSender sender, @NotNull String[] args, @NotNull Object key)
	{
		...
		for (String nickName : Variable.nickNames)
		{
			if (!Method.isUUID(nickName) || Bukkit.getOfflinePlayer(UUID.fromString(nickName)).getName() == null)
			{
				...
				Component hover = offlinePlayer == null
						? null
						: offlinePlayer.getPlayer() != null
								? SenderComponentUtil.senderComponent(offlinePlayer.getPlayer(), Constant.THE_COLOR, true)
	...
	public static List<Completion> tabCompleterList(@NotNull String[] args, @NotNull List<?> list, @NotNull Object key, boolean ignoreEmpty)
	{
		...
		if (!tabArg.isEmpty())
		{
			...
			for (Object o : list)
			{
				...
				{
					returnValue.add(parse(o));
				}
			}
			if (returnValue.size() == 1)
			{
				String s = returnValue.getFirst().suggestion();
			...
			{
				String key2 = (key instanceof Completion completion ? completion.suggestion() : key.toString()).replace("<", "").replace(">", "").replace("[", "")
						.replace("]", "");
			...
			if ((ignoreEmpty && returnValue.isEmpty()) || (!(key instanceof Completion completion ? completion.suggestion() : key.toString()).isEmpty()
					&& returnValue.size() == 1 && returnValue.getFirst().suggestion().equalsIgnoreCase(tabArg)))
			{
				return Collections.singletonList(returnKey);
			}
			return returnValue;
		}
		List<Completion> completions = new ArrayList<>(parse(list, null));
		return sort(completions);
	}

	private static Completion parse(Object o)
	{
		if (o instanceof Completion completion)
		{
			return completion;
		}
		...
		else if (o instanceof Statistic statistic)
		{
		...
		else if (o instanceof UserData userData)
		{
		...
		else if (o instanceof Translatable translatable)
		{
			hover = ComponentUtil.translate(translatable.translationKey());
		}
		return Completion.completion(o.toString().toLowerCase(), hover);
	}
	...
	private static <T> List<Completion> parse(Iterable<?> iterable, @Nullable Predicate<T> exclude)
	{
		...
		{
			if (exclude != null && exclude.test((T) o))
			{
				continue;
			}
			if (o instanceof EnumHideable enumHideable && enumHideable.isHiddenEnum())
			{
				continue;
			}
			completions.add(parse(o));
		}
		return completions;
	}
	...
	public static <T extends Enum<T>> List<Completion> tabCompleterList(@NotNull String[] args, @NotNull Enum<T>[] array, @NotNull Object key,
			boolean ignoreEmpty, @Nullable Predicate<T> exclude)
	{
		return tabCompleterList(args, parse(List.of(array), exclude), key, ignoreEmpty);
	}
	...
	public static List<Completion> tabCompleterBoolean(@NotNull String[] args, @NotNull Object key)
	{
		...
		return CommandTabUtil.tabCompleterList(args, key, false, Completion.completion("true", ComponentUtil.translate(hover)),
				Completion.completion("false", ComponentUtil.translate(hover)));
	}

	@NotNull
	public static List<Completion> tabCompleterDoubleRadius(@NotNull String[] args, double from, double to, @NotNull Object key)
	{
		return tabCompleterDoubleRadius(args, from, false, to, false, key);
	}

	@NotNull
	public static List<Completion> tabCompleterDoubleRadius(@NotNull String[] args, double from, boolean excludeFrom, double to, boolean excludeTo,
			@NotNull Object key)
	{
		String tabArg = args[args.length - 1];
		Completion completion = key instanceof Completion c
				? c
				: Completion.completion(key.toString(),
						ComponentUtil.translate("범위 제한 : %s %s %s %s", Constant.Sosu2.format(from), excludeFrom ? "초과" : "이상", Constant.Sosu2.format(to),
								excludeTo ? "미만" : "이하"));
		if (tabArg.isEmpty())
		{
	...
	public static List<Completion> tabCompleterLongRadius(@NotNull String[] args, long from, long to, @NotNull Object key)
	{
		...
		Completion completion = key instanceof Completion c
				? c
				: Completion.completion(key.toString(), ComponentUtil.translate("범위 제한 : %s 이상 %s 이하", Constant.Sosu2.format(from), Constant.Sosu2.format(to)));
		if (tabArg.isEmpty())
		{
			return Collections.singletonList(completion);
		}
	...
	@NotNull
	public static List<Completion> locationArgument(@NotNull CommandSender sender, @NotNull String[] args, @NotNull Object key, @Nullable Location location,
			boolean isInteger)
	{
	...
	@NotNull
	public static List<Completion> locationArgument(@NotNull CommandSender sender, @NotNull String[] args, @NotNull Object key, @Nullable Location location,
			boolean isInteger, @Nullable List<LocationTooltip> extraSuggestions)
	{
		if (location == null)
		{
		...
		List<Object> list = new ArrayList<>(Arrays.asList(Completion.completion("~ ~ ~"), Completion.completion("~ ~"), Completion.completion("~")));
		...
		if (targetBlock == null || !"$target_block".startsWith(arg))
		{
			if (!arg.isEmpty())
			{
				...
				else if (arg.startsWith("^"))
				{
					...
					list.addAll(Arrays.asList(Completion.completion("^ ^ ^"), Completion.completion("^ ^"), Completion.completion("^")));
				}
	...
	public static List<Completion> rotationArgument(@NotNull CommandSender sender, @NotNull String[] args, @NotNull Object key, @Nullable Location location)
	{
		...
		List<Completion> list = new ArrayList<>(Arrays.asList(Completion.completion("~ ~"), Completion.completion("~"),
	...
	public static List<Completion> worldArgument(@NotNull CommandSender sender, @NotNull String[] args, @NotNull Object key)
	{
		...
		Component tooltip = tooltipString.equals(currentWorld.getName())
				? ComponentUtil.translate("현재 월드 (%s)", currentWorld.getName())
