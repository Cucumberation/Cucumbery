...
{
	public static void manage(int explodePower, float yield, @NotNull List<Block> blockList, @NotNull List<Block> removeList,
			@NotNull List<Block> removeListButNoBreak)
	{
		for (Block block : blockList)
		{
			...
			if (ItemStackUtil.itemExists(itemStack))
			{
				...
				blockTier = nbtItem.hasTag(MiningManager.BLOCK_TIER) && nbtItem.getType(MiningManager.BLOCK_TIER) == NBTType.NBTTagInt ? nbtItem.getInteger(
						MiningManager.BLOCK_TIER) : blockTier;
