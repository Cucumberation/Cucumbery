...
public class Data
{
	...
	public Data(@NotNull CommandSender sender, @NotNull Collection<UUID> targets, HashMap<UUID, Integer> lostAmounts, @NotNull List<UUID> failure,
			@NotNull ItemStack given, int amount)
	{
		this.sender = sender;
	...
	public void sendFeedback(boolean hideOutput)
	{
		...
		if (!failure.isEmpty())
		{
			...
			for (int i = 0; i < failure.size(); i++)
			{
				...
				hover = hover.append(
						ComponentUtil.translate("%s : %s개 (%s - %s)", target, Component.text(amount - lostAmount).color(Constant.THE_COLOR), amountComponent,
								Component.text(lostAmount, Constant.THE_COLOR)));
				if (i + 1 != failure.size())
				{
				...
				MessageUtil.sendWarn(target, "인벤토리가 가득 차서 %s이(가) 보낸 %s %s개 중 %s개를 지급받지 못했습니다", sender, item, amountComponent,
						Component.text(lostAmount, Constant.THE_COLOR));
