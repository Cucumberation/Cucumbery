...
import io.lumine.mythic.bukkit.utils.lib.jooq.impl.QOM.Eq;
...
{
	protected static void setItemLore(@NotNull Material type, @NotNull ItemStack itemStack, @NotNull ItemMeta itemMeta, @NotNull List<Component> originalLore,
			boolean hideAttributes)
	{
		List<Component> lore = new ArrayList<>();
		boolean hasAttributes = itemMeta.hasAttributeModifiers();
		if (!hasAttributes)
			for (EquipmentSlot equipmentSlot : EquipmentSlot.values())
			{
				hasAttributes = hasAttributes || !type.getDefaultAttributeModifiers(equipmentSlot).isEmpty();
			}
		if (hasAttributes)
		{
			...
			if (!hideAttributes)
			{
				for (EquipmentSlot equipmentSlot : EquipmentSlot.values())
				{
					Multimap<Attribute, AttributeModifier> attributeModifiers = itemMeta.hasAttributeModifiers()
							? itemMeta.getAttributeModifiers(equipmentSlot)
							: type.getDefaultAttributeModifiers(equipmentSlot);
					...
					{
						Collection<AttributeModifier> attr = attributeModifiers.get(attribute);
						for (AttributeModifier modifier : attr)
						{
							...
							Component component = ComponentUtil.translate("rgb255,142,82;%s : %s", ComponentUtil.translate(attribute.translationKey()),
									(amount > 0 ? "+" : "") + Constant.Sosu2.format(amount) + operationString);
							lore.add(component);
						}
					}
/*					{
