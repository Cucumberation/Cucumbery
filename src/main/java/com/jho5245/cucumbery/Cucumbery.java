...
//		Cucumbery.using_CommandAPI = Cucumbery.config.getBoolean("use-hook-plugins.CommandAPI") && this.pluginManager.getPlugin("CommandAPI") != null;
		...
		if (Cucumbery.config.getBoolean("console-messages.hook-plugins"))
		{
			if (using_CommandAPI)
			{
				MessageUtil.consoleSendMessage(Prefix.INFO, "CommandAPI 플러그인을 연동했습니다");
			}
	...
	private void registerEvents()
	{
		...
		Initializer.registerEvent(new PlayerArmSwing());
		Initializer.registerEvent(new PlayerBedEnter());
		Initializer.registerEvent(new PlayerBedLeave());
		...
		Initializer.registerEvent(new PlayerKick());
		...
		Initializer.registerEvent(new PlayerPickBlock());
